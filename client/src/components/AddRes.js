...
const AddRes = (res) => {
  const [residential, setRespro] = useState("");
  ...
  const addRes = async () => {
    ...
    try {
      const result = await axios.post("/residential/add", resobj);
      Swal.fire(
        "Success!",
        "The residential project has been created successfully.",
        "success"
      ).then((result) => {
...
// Exporting AddRes.js to Admin.js.
