 ...
 * Fetching the content from http://localhost:8080/residential/delete/:id. Utilizing the Delete method.
 ...
 * Passed onClick() event to delete the residential project.
 * @param {*} e Deleting content that exists in the database via id.
 * @param {*} param0 _id
 * @returns A button icon to delete the residential project.
 */
...
const RemoveRes = ({ _id }) => {
  ...
  const remove = (e, _id) => {
    ...
    axios
      .delete(`/residential/delete/${_id}`)
      .then((response) => {
        const del = res.filter((respro) => _id !== respro._id);
        setRes(del);
        Swal.fire(
          "Success!",
          "The residential project has been deleted successfully.",
          "success"
        ).then(function () {
...
// Exported the RemoveRes.js to Admin.js.
export default RemoveRes;
